/* NOOMA Calendar — biblioteca de icones SVG customizavel + seletor visual.
 * Arquivo separado do app.js para melhor cache do navegador (icones raramente mudam).
 * Carregado como <script> classico ANTES do app.js (type=module), por isso
 * os identificadores abaixo (ICON_SET, renderIcon, etc.) ficam disponiveis
 * globalmente para o app.js consumir sem necessidade de import/export. */

// ================================================================
// ICON LIBRARY -- biblioteca de icones SVG customizavel
// Compativel com dados antigos: se o valor salvo for um emoji,
// renderIcon() mostra o emoji como estava. Se for uma chave
// reconhecida (ex: 'video'), mostra o SVG correspondente.
// ================================================================
const ICON_SET = {
  ...
  'film':'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="2" width="20" height="20" rx="2.18" ry="2.18"/><line x1="7" y1="2" x2="7" y2="22"/><line x1="17" y1="2" x2="17" y2="22"/><line x1="2" y1="12" x2="22" y2="12"/></svg>',
...
// Renderiza um icone: se key existir em ICON_SET, retorna o SVG.
// Se nao existir (ex: emoji legado tipo "\uD83C\uDFAC"), retorna o valor como texto (compatibilidade total).
function renderIcon(key, size){
  size = size||16;
  if(key && ICON_SET[key]){
    return `<span class="nooma-icon" style="display:inline-flex;width:${size}px;height:${size}px;vertical-align:middle">${ICON_SET[key]}</span>`;
  }
  return key||'';
}

// Lista para o seletor de icones (grid do picker)
const ICON_PICKER_KEYS = Object.keys(ICON_SET);

// ================================================================
// ICON PICKER -- seletor visual de icones (portal, como o color picker)
// ================================================================
let _iconPickerPopup=null;
function openIconPicker(anchorEl, currentIcon, onSelect){
  // Fechar picker anterior se existir
  if(_iconPickerPopup){_iconPickerPopup.remove();_iconPickerPopup=null;}

  const popup=document.createElement('div');
  popup.className='icon-picker-popup';
  popup.innerHTML=`
    <div class="icon-picker-hdr">Escolher icone</div>
    <div class="icon-picker-grid">${ICON_PICKER_KEYS.map(k=>
      `<button type="button" class="icon-picker-item${k===currentIcon?' sel':''}" data-icon="${k}" title="${k}">${ICON_SET[k]}</button>`
    ).join('')}</div>
    <div class="icon-picker-emoji-row">
      <input type="text" class="icon-picker-emoji-input" placeholder="ou cole um emoji..." maxlength="4"/>
      <button type="button" class="icon-picker-emoji-btn">Usar</button>
    </div>
  `;
  document.body.appendChild(popup);
  _iconPickerPopup=popup;
  ...
  function position(){
    ...
    if(left<10)left=10;
    if(top+h>window.innerHeight-10)top=rect.top-h-6;
    if(top<10)top=10;
  ...
  popup.querySelectorAll('.icon-picker-item').forEach(btn=>{
    btn.addEventListener('click',()=>{
      ...
      popup.remove();_iconPickerPopup=null;
    });
  });
  const emojiInput=popup.querySelector('.icon-picker-emoji-input');
  popup.querySelector('.icon-picker-emoji-btn')?.addEventListener('click',()=>{
    const v=emojiInput.value.trim();
    if(v){onSelect(v);popup.remove();_iconPickerPopup=null;}
  });
  emojiInput?.addEventListener('keydown',e=>{
    if(e.key==='Enter'){const v=emojiInput.value.trim();if(v){onSelect(v);popup.remove();_iconPickerPopup=null;}}
  });

  window.addEventListener('scroll',()=>{if(_iconPickerPopup===popup)position();},true);
  window.addEventListener('resize',()=>{if(_iconPickerPopup===popup)position();});
  setTimeout(()=>{
    document.addEventListener('click',function handler(ev){
      if(!popup.contains(ev.target)&&ev.target!==anchorEl&&!anchorEl.contains(ev.target)){
        popup.remove();
        if(_iconPickerPopup===popup)_iconPickerPopup=null;
...
// Helper: cria um botao "escolher icone" com preview, que abre o picker.
// containerId: onde inserir o botao. currentIcon: valor inicial. onChange: fn(newIcon)
function makeIconPickerButton(containerId, currentIcon, onChange){
  const container=document.getElementById(containerId);if(!container)return null;
  const btn=document.createElement('button');
  btn.type='button';
  btn.className='icon-picker-trigger';
  btn.innerHTML=renderIcon(currentIcon,18);
  btn.dataset.iconValue=currentIcon||'';
  container.appendChild(btn);
  btn.addEventListener('click',e=>{
    e.stopPropagation();
    openIconPicker(btn,btn.dataset.iconValue,newIcon=>{
      btn.dataset.iconValue=newIcon;
      btn.innerHTML=renderIcon(newIcon,18);
      if(onChange)onChange(newIcon);
    });
  });
  return btn;
}